...
def get_sources(config, arrays, voxel_size, data_files, val=False, graphs=None):
    ...
    if config.get("add_partly"):
        ...
        dt = dt.replace("completely", "partly").replace("complete", "partly")
        overlap_folder_partly = overlap_folder.replace(
            "completely", "partly").replace("complete", "partly")
        if sample_fg_points:
            fg_csv_folder_partly = fg_csv_folder.replace(
                "completely", "partly").replace("complete", "partly")
    ...
    for idx, fn in enumerate(data_files):
        ...
        if config['input_format'] == "hdf":
            ...
            f = zarr.open(fn, 'r')
            required_keys = [config["raw_key"], config.get(
                'gt_key', 'volumes/gt_instances')]
            if overlapping_inst:
                required_keys.append(config.get(
                    'numinst_key', 'volumes/gt_numinst'))
            if not sample_fg_points:
                required_keys.append(config.get(
                    'sample_mask_key', 'volumes/gt_fg_rm_5'))
            missing = [k for k in required_keys if k not in f]
            if missing:
                raise FileNotFoundError(
                    f"{fn} missing {missing}. "
                    "Run flylight/prepare_fisbe_for_ppp.py first.")
            dtype = f[config["raw_key"]].dtype
            num_inst = f['volumes/gt_instances'].shape[0]
        ...
        if overlapping_inst:
            ...
            datasets[gt_numinst] = config.get(
                'numinst_key', 'volumes/gt_numinst')
            ...
            array_specs[gt_instances] = gp.ArraySpec(interpolatable=False,
                    voxel_size=voxel_size)
            if not sample_fg_points:
                datasets[gt_sample_mask] = config.get(
                    'sample_mask_key', 'volumes/gt_fg_rm_5')
    ...
    if overlapping_inst:
        source_parts = [tuple(sources) + gp.RandomProvider()]
        probs = [config["sampling"]["probability_fg"]]

        if len(sources_overlap) > 0:
            source_parts.append(
                tuple(sources_overlap) + gp.RandomProvider())
            probs.append(config["sampling"]["probability_overlap"])
        elif config["sampling"]["probability_overlap"] > 0:
            logger.warning(
                "probability_overlap=%s but no overlap CSVs found in %s; "
                "using foreground sampling only",
        ...
        if len(sources_random) > 0:
            source_parts.append(
                tuple(sources_random) + gp.RandomProvider())
            probs.append(config["sampling"]["probability_random"])
