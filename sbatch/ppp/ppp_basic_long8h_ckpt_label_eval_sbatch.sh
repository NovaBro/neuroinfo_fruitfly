...
# Submit: CKPT=20000 sbatch --export=ALL,CKPT sbatch/ppp/ppp_basic_long8h_ckpt_label_eval_sbatch.sh
#SBATCH --job-name=ppp_basic_long8h_ckpt_vi
...
#SBATCH --output=sbatch/ppp/ppp_basic_long8h_ckpt_vi_%j.out
#SBATCH --error=sbatch/ppp/ppp_basic_long8h_ckpt_vi_%j.err
...
echo "ckpt_label_eval ckpt=${CKPT} vi=vi_th_0_3 job=${SLURM_JOB_ID:-local}"
...
nvidia-smi \
  ...
  --format=csv -l 3 \
  > "gpu_usage_log_ppp_basic_long8h_ckpt_vi_${CKPT}.csv" &
GPU_LOGGER_PID=$!
