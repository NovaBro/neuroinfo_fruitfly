#!/bin/bash
# PatchPerPix fmaps24 train (0 → 80k iters / 8h wall; exp id ppp_basic_fmaps24).
# Auto-resumes from latest checkpoint under metrics/ppp/ppp_basic_fmaps24/train/.
# Prefer train-only first: sbatch sbatch/ppp/ppp_basic_fmaps24_train_sbatch.sh
# Or chain: bash sbatch/ppp/ppp_basic_fmaps24_chain.sh
#SBATCH --job-name=ppp_basic_fmaps24_train
#SBATCH --account=torch_pr_61_general
#SBATCH --gres=gpu:1
# Match training.num_workers=32 in basic_fmaps24.toml
#SBATCH --cpus-per-task=32
#SBATCH --mem=192g
#SBATCH --time=08:00:00
#SBATCH --partition=l40s_public
#SBATCH --output=sbatch/ppp/ppp_basic_fmaps24_train.out
#SBATCH --error=sbatch/ppp/ppp_basic_fmaps24_train.err
...
nvidia-smi \
  ...
  --format=csv -l 3 \
  > gpu_usage_log_ppp_basic_fmaps24_train.csv &
GPU_LOGGER_PID=$!
