#!/bin/bash
# PatchPerPix basic predict/label/evaluate for one val sample (after train).
# Prefer: bash sbatch/ppp/ppp_basic_8h_chain.sh
# Or alone (after train): sbatch sbatch/ppp/ppp_basic_8h_infer_sbatch.sh
#SBATCH --job-name=ppp_basic_infer
...
#SBATCH --output=sbatch/ppp/ppp_basic_infer.out
#SBATCH --error=sbatch/ppp/ppp_basic_infer.err
...
nvidia-smi \
  ...
  --format=csv -l 3 \
  > gpu_usage_log_ppp_basic_infer.csv &
GPU_LOGGER_PID=$!

# No train staging: basic_8h.toml points test_data at scratch fisbe/completely/val.
singularity exec --nv \
