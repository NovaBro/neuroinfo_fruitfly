...
# Submit: CKPT=50000 sbatch --export=ALL,CKPT sbatch/ppp/ppp_basic_fmaps24_ckpt_predict_sbatch.sh
#SBATCH --job-name=ppp_basic_fmaps24_ckpt_predict
...
#SBATCH --output=sbatch/ppp/ppp_basic_fmaps24_ckpt_predict_%j.out
#SBATCH --error=sbatch/ppp/ppp_basic_fmaps24_ckpt_predict_%j.err
...
echo "ckpt_predict ckpt=${CKPT} job=${SLURM_JOB_ID:-local} exp=ppp_basic_fmaps24"
...
nvidia-smi \
  ...
  --format=csv -l 3 \
  > "gpu_usage_log_ppp_basic_fmaps24_ckpt_predict_${CKPT}.csv" &
GPU_LOGGER_PID=$!
