#!/bin/bash
# PatchPerPix long8h: predict/label/evaluate ALL val volumes @ ckpt 70000 (sequential).
...
# Submit: sbatch sbatch/ppp/ppp_basic_long8h_full_val_sbatch.sh
#SBATCH --job-name=ppp_basic_long8h_full_val
...
#SBATCH --output=sbatch/ppp/ppp_basic_long8h_full_val.out
#SBATCH --error=sbatch/ppp/ppp_basic_long8h_full_val.err
...
nvidia-smi \
  ...
  --format=csv -l 3 \
  > gpu_usage_log_ppp_basic_long8h_full_val.csv &
GPU_LOGGER_PID=$!
