#!/bin/bash
# PatchPerPix basic long train (resume → 80k iters / 8h wall; exp id ppp_basic_long8h).
# Auto-resumes from latest checkpoint under metrics/ppp/ppp_basic_long8h/train/.
# Prefer: bash sbatch/ppp/ppp_basic_long8h_chain.sh  (submit AFTER VI sweep review)
# Or alone: sbatch sbatch/ppp/ppp_basic_long8h_train_sbatch.sh
#SBATCH --job-name=ppp_basic_long8h_train
...
#SBATCH --output=sbatch/ppp/ppp_basic_long8h_train.out
#SBATCH --error=sbatch/ppp/ppp_basic_long8h_train.err
...
nvidia-smi \
  ...
  --format=csv -l 3 \
  > gpu_usage_log_ppp_basic_long8h_train.csv &
GPU_LOGGER_PID=$!
